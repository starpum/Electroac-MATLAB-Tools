...
% box_type_inquiry = menu('Select the system type','free air','infinite baffle','closed box','vented box','passive radiator');
...
box_type_inquiry = menu('Select the system type','free air','closed box','vented box');
switch box_type_inquiry
    case 1
        box_type = 0;   %vented box 
    case 2
        box_type = 2;   %vented box 
    case 3
        box_type = 3;   %closed box
...
elseif box_type == 2    % Closed box
    ...
    A=[  - Re/Le,            0,   - Bl/Le,        0;
               0,            0,         1,        0;
          Bl/Mms, -1/(Mms*Cms), - Rms/Mms, - Sd/Mms;
               0,            0,     Sd/Cb,        0];
...
figure(2)
    subplot(211)
semilogx(f,20*log10(abs(u(1)./ffti(1:N/2))))
grid on
xlabel('Frequency (Hz)'); ylabel('Amplitude (dB)');
xlim([50 1500])
